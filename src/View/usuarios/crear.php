...
<html lang="es">
  <head>
    ...
    <title>Crear Usuario - ERP-IA</title>
  ...
  <body>
    <div class="container py-4">
      ...
      <div class="card">
        <div class="card-body">
          <form method="post" action="/usuarios/guardar" class="row g-3">
            <div class="col-md-6">
              <label class="form-label">Nombre</label>
              <input type="text" name="nombre" class="form-control <?= !empty($errors['nombre']) ? 'is-invalid' : '' ?>"
                     value="<?= htmlspecialchars((string)($old['nombre'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" required>
            ...
            <div class="col-md-6">
              ...
              <input type="email" name="email" class="form-control <?= !empty($errors['email']) ? 'is-invalid' : '' ?>"
                     value="<?= htmlspecialchars((string)($old['email'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" required>
            ...
            <div class="col-md-6">
              ...
              <select name="rol_id" class="form-select <?= !empty($errors['rol_id']) ? 'is-invalid' : '' ?>" required>
                ...
                <?php foreach (($roles ?? []) as $r): ?>
                  <option value="<?= (int)$r['id'] ?>" <?= ((string)($old['rol_id'] ?? '') === (string)$r['id']) ? 'selected' : '' ?>>
